...
namespace API.SignalR
{
    public class MessageHub : Hub
    {
        ...
        public MessageHub(IMessageRepository msgRepo, IUserRepository userRepository, IMapper mapper,
        ...
        public override async Task OnConnectedAsync()
        {
            ...
            var messages = await _msgRepo.GetMessageThread(Context.User.GetUsername(), otherUser);
        ...
        public override async Task OnDisconnectedAsync(Exception ex)
        {
            ...
            // if the groupn is empty, it doesn't send anything
            await Clients.Group(group.Name).SendAsync("UpdatedGroup", group);

            await base.OnDisconnectedAsync(ex);
        }

        public async Task SendMessage(CreatedMessageDto createMessageDto)
        {
            var username = Context.User.GetUsername();

            if (username == createMessageDto.RecipientUsername.ToLower())
                throw new HubException("You cannot send messages to yourself bro");
        ...
        private async Task<Group> AddToGroup(string groupName)
        {
            ...
            if (await _msgRepo.SaveAllAsync()) return group;
